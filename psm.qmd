...
r_auditors <- tbl(db, Id(schema = "comp", table = "r_auditors"))
```
...
#| tbl-cap: Sample matched pairs
#| echo: false
#| include: false
#| render: !expr function(x, ...) knitr::knit_print(knitr::kable(x, digits = 3, format.args = list(big.mark = ',')))
sim_matches |> 
  filter(subclass %in% c(220, 481, 333)) |> 
  select(id, big4, mkt_cap, pscore, subclass) |>
  arrange(subclass, big4)
```
...
5. What regression results do you get if you repeat the analysis shown in [Table 25.4](https://iangow.github.io/far_book/psm.html#tbl-lin-reg), but replace `I(mkt_cap / 1e+09)` with `I(mkt_cap^(1/3) * 0.003)`?
