...
#| label: firm_years
#| cache: false
#| dependson: link_table, fundq_local
firm_years <-
  funda_mod |>
  select(gvkey, datadate) |>
  compute()
...
#| label: reg_data
#| cache: false
#| dependson: firm_years, regular_fyears
reg_data <-
  merged_data |>
...
Before graphing results comparable to Table 3 in @Foster:1977wy, let's look at the results in a more raw form.
We address outliers with a `fix_outliers()` function.
...
We then plot a histogram of `abe` for net income for each of the `r length(test_years)` for all six models in @fig-abe-histo.

\indexc{facet\_grid()} \indexc{geom\_histogram()}
```{r}
#| label: fig-abe-histo
...
\indexc{pivot\_wider()} \indexc{row\_number()}
```{r}
model_ranks <-
  results |>
...
2. How do the results in Figure @fig-table_3) compare with those in @Foster:1977wy?
