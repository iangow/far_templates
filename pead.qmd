...
stargazer(fms, type = sg_format, 
          caption = "Regression of fyear on month indicators",
          label = "tab:fyear",
...
#| cache: true
library(parallel)
results <- bind_rows(mclapply(quarters, get_results, mc.cores = 4))
```
...
In making the plot below, note that we set abnormal returns for the first trading day in each plot to zero, so that the plots depict returns starting from day $t-59$ in the first plot and from day $t+1$ in the second plot.
We also add a label to the last day of the series so that we don't need a legend.
