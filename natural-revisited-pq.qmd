...
library(furrr)        # future_map(), future_pmap()
library(broom)        # tidy()
```
...
get_pm <- function(compustat, pm_lag = TRUE, drop_extreme = TRUE) {
...
reg_year_fe <- function(df, dv = "da_adj",
  ...
  suppressWarnings(feols(as.formula(model), 
                         vcov = vcov,
                         notes = FALSE,
                         data = df))
}
...
#| fig-alt: "A plot of coefficients by year and treatment."
#| fig-width: 10
#| warning: false
#| message: false
sho_accruals |>
...
7. Would it make sense, in creating `perf` above, if we instead calculated `ib_at` as `if_else(at > 0, ib / at, NA))`?
